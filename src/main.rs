...
impl Fight {
    fn plan_attack(&mut self, immune_boost: usize) -> HashMap<usize, usize> {
        self.groups.sort_unstable_by(|(_, group_a), (_, group_b)| {
            match group_a
                .get_effective_power(immune_boost)
                .cmp(&group_b.get_effective_power(immune_boost))
            {
                Ordering::Equal => {}
        ...
        for (from, attacker) in &self.groups {
            ...
            let enemy = self
                ...
                .max_by(|(_, group_a), (_, group_b)| {
                    let damage_to_a = attacker.get_damage_to(group_a, immune_boost);
                    let damage_to_b = attacker.get_damage_to(group_b, immune_boost);
                    match damage_to_a.cmp(&damage_to_b) {
                        Ordering::Equal => {}
                        ord => return ord,
                    }

                    let power_a = group_a.get_effective_power(immune_boost);
                    let power_b = group_b.get_effective_power(immune_boost);
            ...
            if let Some((to, target)) = enemy {
                if attacker.get_damage_to(target, immune_boost) > 0 {
    ...
    fn attack(&mut self, plan: &HashMap<usize, usize>, immune_boost: usize) {
        self.groups.sort_unstable_by(|(_, group_a), (_, group_b)| {
        ...
        for ref attacker_id in attackers {
            ...
            let damage = attack_group.get_damage_to(target_group, immune_boost);
    ...
    fn run_fight(&mut self, immune_boost: usize) -> Option<GroupKind> {
        loop {
            let count_of_immune = self
                ...
                .count();
            if count_of_immune == 0 {
                return Some(GroupKind::Infection);
            }
            ...
            let count_of_infection = self
                ...
                .count();
            if count_of_infection == 0 {
                return Some(GroupKind::Immune);
            }

            let old_groups = self.groups.clone();
            let plan = self.plan_attack(immune_boost);
            self.attack(&plan, immune_boost);

            if old_groups == self.groups {
                return None;
            }
        }
    }
...
fn main() {
    ...
    loop {
        ...
        let winner = fight.run_fight(boost);
        println!("Boost: {}, winner: {:?}", boost, winner);

        if let Some(GroupKind::Immune) = winner {
            break;
        }
        boost += 1;
    }
    ...
    let win_units: usize = fight
        ...
        .sum();
    println!("Winner: {:?}", fight.groups[0].1.kind);
    println!("Sum of winner units: {}", win_units);
    println!("Groups: {:#?}", fight.groups);
}
